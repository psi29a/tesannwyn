...
int HumptyImage(char *output_filename, int opt_image_type, int bpp, int opt_adjust_height, int opt_grid,
    ...
    for (y = min_y; y <= max_y; y++) {
        for (x = min_x; x <= max_x; x++) {
            ...
            if ((fp_land = fopen(land_filename, "rb")) == 0) {
                 ...
                 ***********************************************************/
                for (c = 1; c < ppc + 1; c++) {
                    fseek(fp_o, 54 + ((y - min_y) * xdim * 3 * ppc) + ((x - min_x) * 3 * ppc) + ((c - 1) * xdim * 3),
                          SEEK_SET);
                    for (i = 1; i < ppc + 1; i++) {
                        rgb = 0xFFFFFFFF;
                        if (opt_grid != -1) {
    ...
    for (y = min_y; y <= max_y; y++) {
        for (x = min_x; x <= max_x; x++) {
            ...
            if ((fp_land = fopen(land_filename, "rb")) == 0) {
                ...
                for (i = 0; i < ppc; i++) {
                    fseek(fp_o, 54 + ((y - min_y) * (xdim * Bp * ppc + (ppc * opad))) + ((x - min_x) * Bp * ppc) +
                                ((i) * xdim * Bp) + (i * opad), SEEK_SET);
                    for (j = 0; j < ppc; j++) {
                        fwrite(&tex, 3, 1, fp_o);
                    }
...
int ExportImages(int opt_image_type, int opt_bpp, int opt_vclr, int opt_grid, int opt_adjust_height,
    ...
    printf("\n\nHighest point was %d THU (%d Game Units) = %f metres    [Cell (%d,%d)]\n",
           height_stat.max, height_stat.max * 8, ((float) height_stat.max * 0.114), height_stat.cell_max_x,
           height_stat.cell_max_y);
    printf("Lowest  point was %d THU (%d Game Units) = %f metres    [Cell (%d,%d)]\n",
           height_stat.min, height_stat.min * 8, ((float) height_stat.min * 0.114), height_stat.cell_min_x,
           height_stat.cell_min_y);
...
int ExportTES3Land(char *input_esp_filename, int opt_vclr, int cleanup_list_x[], int cleanup_list_y[],
    ...
    while (fread(s, 1, 16, fpin) > 0) {
        ...
        if (or == NULL) {
            fprintf(stderr, "Unable to allocate %u bytes of"
